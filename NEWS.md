...
* The Viewer pane preview shown when you `print()` a chart now shows a one-off console reminder, the first time it actually needs to fast-place a `plot_label()`, that the preview uses a quick placement heuristic rather than the real collision-avoiding search - labels can overlap there even when `save_e61()` would place them cleanly. Controlled by the new `theme61.autolabel_fast_msg` option (default `TRUE`); turns itself off after showing once, and can be disabled entirely via `set_t61_options(list(theme61.autolabel_fast_msg = FALSE))`.
* Added a `build_up` argument to `save_e61()` for single-panel graphs. When `TRUE`, saves a sequence of files (`filename_1`, `filename_2`, ..., `filename_N`) that each reveal one more category/series than the last, for stepping a chart across several PowerPoint slides (closes #253). Categories not yet revealed are blanked (zeroed, or set to missing) rather than removed from the data, so the axes, scales and dimensions are computed once and stay identical across every step. Supports bar/column charts (reveals x-axis categories left to right), stacked area/ribbon charts (reveals the stacked groups bottom to top), and grouped line/point/`geom_pointbar()` charts (reveals one colour/fill group at a time); a single, ungrouped line or area series instead builds up progressively along the x-axis, with the number of steps controlled by the new `build_up_n` argument. `plot_label()` labels are synced to the reveal sequence too, matched to a category by colour. Not supported for multi-panel or faceted graphs.
...
* `save_e61()`'s ~30 top-level arguments have been grouped into four named lists, following the same pattern already used by `dim`: `labs` (`title`, `subtitle`, `footnotes`, `sources`), `layout` (`ncol`, `nrow`, `align`, `axis`) and `spacing` (`pad_width`, `pad_height`, `outer_width`, `outer_height`, `height_adj`, `rel_heights`, and the old `spacing_adj`'s `title`/`subtitle`, now folded directly into `spacing`). The old top-level arguments still work but are deprecated and will warn - update calls like `save_e61(..., title = "x", ncol = 3, pad_width = 5)` to `save_e61(..., labs = list(title = "x"), layout = list(ncol = 3), spacing = list(pad_width = 5))`.
