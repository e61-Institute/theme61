...
- Format the x and y axes scales: `scale_y_continuous_e61()`, `scale_x_continuous_e61()`. Your y-axis should get automatically scaled when you save your chart using `save_e61()` but if you do not like the automatic scaling then you can always scale your axes manually.
...
Implementing dual y-axis requires some additional faff that I will try and step through.
...
You must then specify the `sec_axis` argument in `scale_y_continuous_e61()` with `sec_axis(~sec_rescale(.))`.

You also need to set `rescale_sec = TRUE` to ensure the formatting is appropriately adjusted for the secondary axis.

Finally, you will need to run the graph code twice, as the first time, there will be an error caused by how R internally assigns objects generated by simultaneous function calls (this may get fixed in the future). 
...
```{r, include=F, echo=F}
# Rendering the graph twice to make the second axis appear?
g
g
```
