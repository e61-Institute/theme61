...
You must then pass `sec_rescale_axis()` to the `sec_axis` argument in `scale_y_continuous_e61()`, giving it the same `scale` and `shift` values you gave `sec_rescale_inv()`.

```{r}
g <- ggplot(data_dual, aes(x = year)) + 
  geom_line(aes(y = `Food and Tobacco`), colour = palette_e61(2)[[1]]) +
  # All the action is in the next 4 lines
  ...
  scale_y_continuous_e61(limits = c(0, 100, 25), 
                         sec_axis = sec_rescale_axis(scale = 0.1, shift = 0)) +
