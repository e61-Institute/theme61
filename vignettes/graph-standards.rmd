...
We start by creating a simple graph using the 'normal' stuff: `ggplot(data, aes(x, y)) + geom_blah()`

Then:
-   [(optional) Manually add graph colours if they are not applied by default](#colours): `scale_colour_e61()` or `scale_fill_e61()`
-   [(optional) Manually format the x and y axes scales](#scales): `scale_y_continuous_e61()`, `scale_x_continuous_e61()`
...
We're start with a simple ggplot2 graph with no labels.
...
```{r, echo = FALSE}
ggsave(here::here("man/figures/g-prev.svg"), gg)
knitr::include_graphics(here::here("man/figures/g-prev.svg"))
```

## Applying the e61 theme to the graph {#theme}

Most of the theming is done by the `theme_e61()` function. This is included automatically in `theme61::ggplot()`.
...
`theme61` comes with a comprehensive colour palette that you can use in your colour and fill scales. are automatically added when you use `theme61::ggplot()` and add either a fill or colour aesthetic variable. The colour palette also comes with colour and fill functions (`scale_colour_e61()` and `scale_fill_e61()`) if you want to add them to other plots or you do not define your colour and fill variables in the main `ggplot` call (e.g. if you were to define them within an `aes` call in `geom_line()`).
...
`scale_y_continuous_e61()` is the preferred axis scale function for graphs. It duplicates the axis by default and allows you to specify the axis limits in the `limits` argument if you do not like the defaults applied in `save_e61()`.
...
The package also defaults to showing no x-axis title. This is because the x-axis is usually very obvious. For example, if the x-axis is "2010, 2011, 2012, ...", then it is obviously a time series in years, which would be a waste of space to label. Save the x-axis labels for when the x-axis actually requires explanation.
...
You do not need to supply the ggplot object when using `save_e61()` as it automatically saves the last-created graph.
...
g2 <- ggplot(data_dual, aes(x = year)) +
  ...
  labs_e61(title = "Selected Aggregates")
