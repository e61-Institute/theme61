...
## Alternatives when things are fiddly

There are two situations where using `plot_label()` may not work. When labeling faceted graphs or when wanting to add a lot of labels.

### Facets

When you have a faceted graph, `plot_label()` will not work properly. This is because the annotations will appear in the same position on all the facets and there is no way to prevent this. This is problematic because the labels are unnecessarily repeating themselves, and the correct position of the text on one facet may be different on each facet (e.g. if the height of the columns differs across facets).
...
save_e61(here::here("man/figures/label-facet.svg"), graph)
knitr::include_graphics(here::here("man/figures/label-facet.svg"))
