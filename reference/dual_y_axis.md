...
claims this is to prevent people from abusing secondary y-axis to show
dodgy correlations (investment banking-style). However, sometimes they
have valid uses and the following functions enable this with as little
fiddliness as possible.

## Usage

``` r
sec_rescale_inv(values, scale = 1, shift = 0)

sec_rescale(
  values,
  scale = t61_env$sec_axis_scale,
  shift = t61_env$sec_axis_shift
)
```
...
- values:
  ...
  Vector of data that would normally be passed as the y aesthetic in the
  graph.

- scale:

  Numeric. Multiplicative factor that rescales the axis. For example, if
  the scale was originally 0 to 50, then `scale = 0.1` would rescale
  this to 0 to 5.

- shift:

  Numeric. Moves the axis up and down. For example, if the scale was 0
  to 5, `shift = 5` moves the secondary scale down by 5 units to range
  from -5 to 0.
...
See the examples for how to use the two functions to manipulate the
secondary axis. Trial and error will be needed to select appropriate
